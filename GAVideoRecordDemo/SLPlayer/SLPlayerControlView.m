...
- (void)changeResolution:(UIButton *)sender {
    ...
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:resolutionAction:)]) {
        [self.sl_delegate sl_controlView:self resolutionAction:sender];
    }
}
...
- (void)tapSliderAction:(UITapGestureRecognizer *)tap {
    if ([tap.view isKindOfClass:[UISlider class]]) {
        ...
        if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:progressSliderTap:)]) {
            [self.sl_delegate sl_controlView:self progressSliderTap:tapValue];
        }
    }
}
...
- (void)backBtnClick:(UIButton *)sender {
    ...
    if (self.isCellVideo && orientation == UIInterfaceOrientationPortrait) {
        if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:closeAction:)]) {
            [self.sl_delegate sl_controlView:self closeAction:sender];
        }
    } else {
        if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:backAction:)]) {
            [self.sl_delegate sl_controlView:self backAction:sender];
        }
    }
}
...
- (void)lockScrrenBtnClick:(UIButton *)sender {
    ...
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:lockScreenAction:)]) {
        [self.sl_delegate sl_controlView:self lockScreenAction:sender];
    }
}

- (void)playBtnClick:(UIButton *)sender {
    sender.selected = !sender.selected;
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:playAction:)]) {
        [self.sl_delegate sl_controlView:self playAction:sender];
    }
}
...
- (void)fullScreenBtnClick:(UIButton *)sender {
    sender.selected = !sender.selected;
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:fullScreenAction:)]) {
        [self.sl_delegate sl_controlView:self fullScreenAction:sender];
    }
}
...
- (void)repeatBtnClick:(UIButton *)sender {
    [self sl_playerResetControlView];
    [self sl_playerShowControlView];
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:repeatPlayAction:)]) {
        [self.sl_delegate sl_controlView:self repeatPlayAction:sender];
    }
}

- (void)downloadBtnClick:(UIButton *)sender {
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:downloadVideoAction:)]) {
        [self.sl_delegate sl_controlView:self downloadVideoAction:sender];
    }
}
...
- (void)centerPlayBtnClick:(UIButton *)sender {
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:cneterPlayAction:)]) {
        [self.sl_delegate sl_controlView:self cneterPlayAction:sender];
    }
}

- (void)failBtnClick:(UIButton *)sender {
    self.failBtn.hidden = YES;
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:failAction:)]) {
        [self.sl_delegate sl_controlView:self failAction:sender];
    }
}
...
- (void)progressSliderTouchBegan:(UISlider *)sender {
    ...
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:progressSliderTouchBegan:)]) {
        [self.sl_delegate sl_controlView:self progressSliderTouchBegan:sender];
    }
}

- (void)progressSliderValueChanged:(UISlider *)sender {
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:progressSliderValueChanged:)]) {
        [self.sl_delegate sl_controlView:self progressSliderValueChanged:sender];
    }
}
...
- (void)progressSliderTouchEnded:(UISlider *)sender {
    self.showing = YES;
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlView:progressSliderTouchEnded:)]) {
        [self.sl_delegate sl_controlView:self progressSliderTouchEnded:sender];
    }
}
...
- (void)sl_playerShowControlView {
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlViewWillShow:isFullscreen:)]) {
        [self.sl_delegate sl_controlViewWillShow:self isFullscreen:self.isFullScreen];
    }
    [self sl_playerCancelAutoFadeOutControlView];
...
- (void)sl_playerHideControlView {
    if ([self.sl_delegate respondsToSelector:@selector(sl_controlViewWillHidden:isFullscreen:)]) {
        [self.sl_delegate sl_controlViewWillHidden:self isFullscreen:self.isFullScreen];
    }
    [self sl_playerCancelAutoFadeOutControlView];
    [UIView animateWithDuration:SLPlayerControlBarAutoFadeOutTimeInterval animations:^{
        [self hideControlView];
